...
/* === PATCH: Deterministic dot logic & robust mode toggle (Kevin spec) ===
   Mapping:
   - NO INFO => 'standby' (orange)
   - INFO & MARKET OPEN => 'open' (green)
   - INFO & MARKET CLOSED => 'closed' (red)
   Guarantees:
   - Start in Simulation (no API calls).
   - Toggling to Real starts API polling and per‑tile requests.
   - Toggling back to Simulation aborts in‑flight requests and stops polling.
   - No 'pre'/'post' intermediate states used for dots.
*/
(function(){
  const W = window, D = document;
  // --- Helpers
  function getAssetByTicker(t){ return (W.assets || []).find(x => x.ticker === t); }
  function getAssetByIndex(i){ return (W.assets || [])[i]; }
  function tileIndex(tile){ return Number(tile && tile.dataset && tile.dataset.index || -1); }
  function markInfo(asset){ if(asset){ asset._hasInfo = true; asset._lastQuoteTs = Date.now(); } }
  function clearInfo(asset){ if(asset){ asset._hasInfo = false; asset._lastQuoteTs = 0; } }

  // --- Market status normalization
  function isMarketOpenForTicker(ticker){
    const FH = W.FINNHUB || {};
    const ex = (FH.exchangeMap && FH.exchangeMap[ticker]) || FH.defaultExchange || 'US';
    const st = FH.marketStatus && FH.marketStatus[ex];
    if(!st) return false;
    // Normalize across possible shapes
    if (st.isOpen === true) return true;
    if (st.market === 'open') return true;
    if (st.session === 'open') return true;
    return false;
  }

  // --- Compute dot state per tile
  function computeDotState(tile){
    // Simulation => always NO INFO (standby / orange)
    if (!W.realMode) return 'standby';
    const idx = tileIndex(tile);
    const a = getAssetByIndex(idx);
    if (!a || !a._hasInfo) return 'standby';
    return isMarketOpenForTicker(a.ticker) ? 'open' : 'closed';
  }

  // --- Update a single tile dot deterministically
  function setTileDot(tile){
    if (!tile) return;
    let dot = tile.querySelector('.status-dot');
    if (!dot){
      dot = D.createElement('span');
      dot.className = 'status-dot standby';
      tile.appendChild(dot);
    }
    const state = computeDotState(tile); // 'standby' | 'open' | 'closed'
    // Reset classes
    dot.classList.remove('open','closed','pre','post','pulsing','standby');
    dot.classList.add(state);
    // Pulse only when open (market streaming)
    if (state === 'open') dot.classList.add('pulsing');
  }

  function updateAllDots(){
    D.querySelectorAll('.asset-tile').forEach(setTileDot);
  }

  // Wire our dot updater to expose in global for reuse
  W.__updateAllDots = updateAllDots;
  W.__setTileDot = setTileDot;

  // --- Wrap quote fetch to mark info
  try {
    const _origFetch = W.fetchFinnhubQuote;
    if (typeof _origFetch === 'function'){
      W.fetchFinnhubQuote = async function(ticker, signal){
        const res = await _origFetch(ticker, signal);
        if (res && typeof res.c === 'number' && res.c > 0){
          const a = getAssetByTicker(ticker);
          markInfo(a);
          // Also refresh that tile dot now
          const idx = (W.assets||[]).findIndex(x=>x.ticker===ticker);
          const tile = D.querySelector(`.asset-tile[data-index="${idx}"]`);
          if (tile) setTileDot(tile);
        }
        return res;
      };
    }
  ...
  // --- Centralized API scheduler with cancellation & concurrency cap
  const ApiManager = {
    _interval: null,
    _controller: null,
    _busy: false,
    periodMs: (W.FINNHUB && W.FINNHUB.rateMs) || 12000,
    maxConcurrent: 5,
    start(){
      this.stop(); // ensure clean
      if (!W.realMode) return;
      const tick = async () => {
        if (!W.realMode || this._busy) return;
        this._busy = true;
        try{
          // Abort any leftover
          ...
          const list = (W.assets || []).filter(a => a && a.live !== false); // default live
          // Concurrency-limited runner
          let i = 0, active = 0;
          await new Promise((resolve) => {
            const pump = () => {
              if (i >= list.length && active === 0) return resolve();
              while (active < ApiManager.maxConcurrent && i < list.length){
                const a = list[i++];
                active++;
                (async () => {
                  try {
                    if (typeof W.fetchFinnhubQuote === 'function'){
                      await W.fetchFinnhubQuote(a.ticker, sig);
                    }
                  } finally {
                    active--;
                    setTimeout(pump, 0);
          ...
          // After batch, refresh dots
          updateAllDots();
        } finally {
          this._busy = false;
        }
      };
      // Kick once immediately and then schedule
      tick();
      this._interval = setInterval(tick, this.periodMs);
    },
    stop(){
      if (this._interval) { clearInterval(this._interval); this._interval = null; }
      if (this._controller) { try{ this._controller.abort(); }catch(_e){} this._controller = null; }
    }
  };
  W.__ApiManager = ApiManager;

  // --- Override applyMode to drive setMode and our scheduler
  const _origApplyMode = W.applyMode;
  W.applyMode = function(checked){
    try{ if (typeof _origApplyMode === 'function') _origApplyMode(checked); }catch(_e){}
    W.setMode && W.setMode(!!checked);
  };

  // --- Extend setMode to enforce lifecycle and dot mapping
  const _origSetMode = W.setMode || function(){};
  W.setMode = function(useReal){
    W.realMode = !!useReal;
    // call original (for UI, labels, etc.)
    try{ _origSetMode.call(W, W.realMode); }catch(_e){}
    // lifecycle
    if (W.realMode){
      // Switch from simulation -> real
      // Keep existing simulated prices but dots start as NO INFO
      (W.assets || []).forEach(clearInfo);
      updateAllDots();
      ApiManager.start();
    } else {
      // Switch to simulation: stop API & mark NO INFO (orange)
      ApiManager.stop();
      (W.assets || []).forEach(clearInfo);
      updateAllDots();
    }
    // Emit a custom event (for any other listeners)
    try{
      const ev = new Event('modechange');
      W.dispatchEvent(ev);
    }catch(_e){}
  };

  // --- Ensure initial state: Simulation (real data off) & all dots orange
  if (typeof W.realMode !== 'boolean') W.realMode = false;
  (W.assets || []).forEach(clearInfo);
  if (D.readyState === 'loading'){
    D.addEventListener('DOMContentLoaded', updateAllDots, { once:true });
  } else {
    updateAllDots();
  }

  // --- Refresh dots whenever market status loop ticks (if other code emits custom events)
  W.addEventListener && W.addEventListener('marketstatus', updateAllDots);
  // Also poll a light dot refresh every 10s to keep colors consistent with market status
  setInterval(updateAllDots, 10000);
})();
// === END PATCH ===


// === PATCH: redirect legacy startLiveRefreshLoop to ApiManager ===
if (typeof window.startLiveRefreshLoop === 'function'){
  window.startLiveRefreshLoop = function(){ 
    if (window.__ApiManager){ window.__ApiManager.start(); }
  };
} else {
  // define it for callers
  window.startLiveRefreshLoop = function(){
    if (window.__ApiManager){ window.__ApiManager.start(); }
  };
}
...
// === PATCH: override legacy dot updaters to enforce 3-color mapping ===
if (typeof window.updateTileStatusDot === 'function'){
  window.updateTileStatusDot = function(tile){ 
    if (window.__setTileDot) return window.__setTileDot(tile);
  };
}
if (typeof window.updateAllStatusDots === 'function'){
  window.updateAllStatusDots = function(){
    if (window.__updateAllDots) return window.__updateAllDots();
  };
}
